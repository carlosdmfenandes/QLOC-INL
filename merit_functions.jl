...
"Defines merit functions for post-selected nonlinear optical processes."
mer
"Calculates the success transition amplitude for n input photons
...
merit_stddev(parray) = std(a=bs.(parray), false)

merit_lprob(parray, c=1.) = merit_stddev(parray) + c*(1-mean(parray))^2

merit_centre(parray, c=1., d=1.) = merit_lprob(parray, c) +  d*sin(nlarg)^2
